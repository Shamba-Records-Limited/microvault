...
- **`sweep_foreign_asset` cannot drain the underlying asset.** The function compares `token_to_recover` against `Vault::query_asset(e)` and rejects the call. Foreign tokens (e.g. a SEP-41 token sent by mistake) can be recovered by the treasury.
